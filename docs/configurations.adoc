...
|config-loader
|The config loader object that will provide configurations to be merged into the base configuration at the start of the pipeline and for every micro-batch.
Note this is typically only required when streaming pipelines require dynamic refreshing of configurations.
...
|type
|The deriver type to be used. Envelope provides `morphline`, `nest`, `passthrough`, `sql`, `pivot`, `exclude`, `distinct`, `select`, `in-list`, `hash`, `latest`, `dq`, `translate`, `sparkml`, `parse-json`.
...
|as-struct
|Whether to place the parsed fields within their own struct field, instead of directly on the record.
Default false.
...
|schema
|The schema of the JSON records. Refer to the link:configurations.adoc#schema[Schema documentation]

|option.*
|Passes through Spark-specific JSON options to Spark.
For example, `option.mode = FAILFAST` will set the option `mode` to `FAILFAST`.
Refer to the link:https://spark.apache.org/docs/latest/api/java/org/apache/spark/sql/DataFrameReader.html#json-scala.collection.Seq-[Spark JSON documentation] for available options.
