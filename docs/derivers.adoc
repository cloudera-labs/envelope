...
Envelope provides five derivers out of the box and also allows custom derivers to be specified.
...
The `morphline` deriver is used to run Morphline transformations over the records of the single dependency of the step.
...
To reference the deriver in your pipeline simply use the deriver's fully qualified class name as the deriver type. For example:
