...
import com.cloudera.labs.envelope.input.stream.translate.Translator;
...
public class KafkaInput extends StreamInput {
    ...
    public KafkaInput(Config config) throws Exception {
        super(config);
    }
    ...
    public JavaDStream<Row> getDStream() throws Exception {
        ...
        if (encoding.equals("string")) {
            ...
            dStream = stringDStream.map(new Translator.TranslateFunction<String>(translatorConfig));
        }
            ...
            dStream = byteArrayDStream.map(new Translator.TranslateFunction<byte[]>(translatorConfig));
        }
        else {
    ...
    public StructType getSchema() throws Exception {
        Config translatorConfig = config.getConfig("translator");
        return Translator.translatorFor(translatorConfig).getSchema();
    }
